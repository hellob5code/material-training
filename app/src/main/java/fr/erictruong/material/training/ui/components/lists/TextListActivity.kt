...
class TextListActivity : RecyclerActivity() {
    ...
    private fun mapOneLineItem(model: DummyModel): MaterialListItem = TextItem(
            id = model.id,
            text1 = "Single-line item",
            action = { v, item -> Snackbar.make(recyclerView, "Action - " + item, Snackbar.LENGTH_SHORT).show() })

    private fun mapTwoLineItem(model: DummyModel): MaterialListItem = TextItem(
            id = model.id,
    ...
    private fun mapThreeLineItem(model: DummyModel): MaterialListItem = TextItem(
            id = model.id,
